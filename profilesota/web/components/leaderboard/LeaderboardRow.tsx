...
export function LeaderboardRow({ entry, isCurrentUser }: LeaderboardRowProps) {
  const name = displayName(
    { handle: entry.handle, displayName: entry.displayName, wallet: entry.wallet },
    entry.wallet,
  );

  return (
    <div
      className={`flex items-center gap-4 py-3 px-4 border-b border-[#1a1a1a] transition-colors ${
        isCurrentUser ? 'bg-[#00ff88]/5 border-l-2 border-l-[#00ff88]' : ''
      }`}
    >
      {/* Rank */}
