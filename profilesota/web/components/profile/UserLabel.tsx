...
interface UserLabelProps { label: UserLabelData; size?: 'sm' | 'md'; }

export function UserLabel({ label, size = 'sm' }: UserLabelProps) {
  const style = LABEL_STYLES[label.label] || { bg: 'rgba(255,255,255,0.05)', text: label.color || '#e0e0e0', icon: '' };
  const textSize = size === 'sm' ? 'text-[10px]' : 'text-xs';
  const padding = size === 'sm' ? 'px-1.5 py-0.5' : 'px-2 py-0.5';
  return (
    <span className={`inline-flex items-center gap-1 rounded-full font-mono font-medium ${textSize} ${padding}`}
