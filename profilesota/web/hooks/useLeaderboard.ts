...
interface LeaderboardResponse extends PaginatedResponse<LeaderboardEntry> { category: string; updatedAt: string; }

export function useLeaderboard(type: LeaderboardType = 'volume', page: number = 1) {
  return useQuery({
    queryKey: queryKeys.leaderboard.byType(type, page),
    queryFn: () => api.get<LeaderboardResponse>(`/leaderboard/${type}?page=${page}&pageSize=25`),
    staleTime: 60_000,
