...
export function ProfilePage({ handle, isOwnProfile, onEditClick }: ProfilePageProps) {
  const [activeTab, setActiveTab] = useState<Tab>('stats');
  const { data: profile, isLoading: profileLoading } = useProfile(handle);
  ...
  if (profileLoading) {
    return (
      <div className="max-w-3xl mx-auto p-6 space-y-4">
        ...
        <div className="h-48 bg-[#111111] border border-[#1a1a1a] rounded-lg animate-pulse" />
      </div>
    );
  }
  ...
  const cardData: ProfileCardData = {
    ...
    xpToNextLevel: profile.stats?.xpToNextLevel || 100,
    totalBets: profile.stats?.totalBets || 0,
    totalVolume: profile.stats?.totalVolume || '0',
    totalPnl: profile.stats?.totalPnl || '0',
    ...
    badges: (profile.badges || [])
      .filter((b) => b.earnedAt)
      .slice(0, 5)
      .map((b) => ({ slug: b.slug, name: b.name, rarity: b.rarity, imageUrl: b.imageUrl })),
    ...
    <div className="max-w-3xl mx-auto p-6 space-y-4">
          ...
          <button
            ...
            className={`text-xs font-mono px-4 py-2 border-b-2 transition-colors ${
              activeTab === tab.key
                ...
                : 'text-[#666666] border-transparent hover:text-[#e0e0e0]'
            }`}
          >
            {tab.label}
          </button>
      ...
      <div>
        ...
        {activeTab === 'history' && profile.id && (
          <BetHistory userId={profile.id} />
        )}

        {activeTab === 'badges' && badges && (
          <ProfileBadges badges={badges} layout="grid" showLocked />
        )}
      </div>
    </div>
  );
