...
interface MarketResolvedPayload {
  ...
  bets: Array<{
    ...
    rangeIndex: number;
    rangeLabel?: string;
    amount: string; // wei or ETH string
    txHash: string;
    claimed: boolean;
    claimAmount?: string;
  }>;
...
export class WebhookService {
  ...
  async handleMarketResolved(payload: MarketResolvedPayload) {
    this.logger.log(
      `Processing market ${payload.marketAddress} with ${payload.bets.length} bets`,
    );
    ...
    for (const bet of payload.bets) {
      ...
      const pnl = won && claimAmount
        ? (parseFloat(claimAmount) - parseFloat(amount)).toString()
      ...
      const event: BetResolvedEvent = {
        ...
        rangeLabel: bet.rangeLabel || `Range ${bet.rangeIndex}`,
        amount,
        txHash: bet.txHash,
        won,
        claimAmount,
      ...
      try {
        this.events.emit('bet.resolved', event);
        processed++;
      } catch (err) {
        this.logger.warn(
          `Failed to process bet ${bet.txHash}: ${err}`,
        );
        skipped++;
      }
    }
    ...
    this.logger.log(
      `Market ${payload.marketAddress}: ${processed} processed, ${skipped} skipped`,
    );
