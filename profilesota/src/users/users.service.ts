...
export class UsersService {
  ...
  async getCard(userId: string) {
    const user = await this.prisma.user.findUnique({
      ...
      include: {
        ...
        stats: {
          select: {
            totalBets: true, totalWins: true, totalLosses: true,
            totalVolume: true, totalPnl: true, bestStreak: true, xp: true, level: true,
          },
        },
        labels: true,
        badges: {
  ...
  async getBatch(addresses: string[]) {
    const normalized = addresses.slice(0, 50).map((a) => a.toLowerCase());

    const users = await this.prisma.user.findMany({
      where: { wallet: { in: normalized } },
      include: {
        profile: true,
        stats: { select: { xp: true, level: true, totalBets: true } },
        labels: true,
        badges: {
          where: { displayed: true },
          include: { badge: true },
  ...
  private formatFullProfile(user: any) {
    ...
    return {
      id: user.id, wallet: user.wallet, createdAt: user.createdAt, lastSeenAt: user.lastSeenAt,
      profile: user.profile ? {
        handle: user.profile.handle, displayName: user.profile.displayName,
        bio: user.profile.bio, avatarUrl: user.profile.avatarUrl, isPublic: user.profile.isPublic,
      } : null,
      stats: stats ? {
        ...
        xp: stats.xp, level: stats.level, xpToNextLevel: this.xpForLevel(stats.level + 1) - stats.xp,
      } : null,
      labels: (user.labels || []).map((l: any) => ({ label: l.label, color: l.color, icon: l.icon })),
      badges: (user.badges || []).map((ub: any) => ({
        slug: ub.badge.slug, name: ub.badge.name, description: ub.badge.description,
        imageUrl: ub.badge.imageUrl, rarity: ub.badge.rarity, category: ub.badge.category,
        earnedAt: ub.earnedAt, displayed: ub.displayed,
      })),
  ...
  private formatCardProfile(user: any) {
    ...
    return {
      ...
      handle: user.profile?.handle || null, displayName: user.profile?.displayName || null,
      avatarUrl: user.profile?.avatarUrl || null,
      level: stats?.level || 1, xp: stats?.xp || 0,
      xpToNextLevel: stats ? this.xpForLevel(stats.level + 1) - stats.xp : 100,
      totalBets: stats?.totalBets || 0, totalVolume: stats?.totalVolume?.toString() || '0',
      totalPnl: stats?.totalPnl?.toString() || '0', winRate, bestStreak: stats?.bestStreak || 0,
      labels: (user.labels || []).map((l: any) => ({ label: l.label, color: l.color, icon: l.icon })),
      badges: (user.badges || []).map((ub: any) => ({
        slug: ub.badge.slug, name: ub.badge.name, rarity: ub.badge.rarity, imageUrl: ub.badge.imageUrl,
      })),
  ...
  private formatMiniProfile(user: any) {
    return {
      id: user.id, wallet: user.wallet,
      handle: user.profile?.handle || null, displayName: user.profile?.displayName || null,
      avatarUrl: user.profile?.avatarUrl || null, level: user.stats?.level || 1,
      labels: (user.labels || []).map((l: any) => ({ label: l.label, color: l.color, icon: l.icon })),
      badges: (user.badges || []).map((ub: any) => ({ slug: ub.badge.slug, name: ub.badge.name, rarity: ub.badge.rarity })),
    };
  }
