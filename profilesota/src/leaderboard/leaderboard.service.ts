...
const CATEGORY_FIELD: Record<LeaderboardCategory, string> = {
  volume: 'totalVolume', pnl: 'totalPnl', wins: 'totalWins', streak: 'bestStreak',
};
...
export class LeaderboardService {
  ...
  private async refreshCategory(category: LeaderboardCategory) {
    ...
    const entries = topStats.map((s, i) => ({
      rank: i + 1, userId: s.userId, wallet: s.user.wallet,
      handle: s.user.profile?.handle || null, displayName: s.user.profile?.displayName || null,
      avatarUrl: s.user.profile?.avatarUrl || null, level: s.level,
      value: (s as any)[field]?.toString() || '0', labels: s.user.labels,
    }));

    await this.prisma.leaderboardSnapshot.create({ data: { category, entries: entries as any } });

    const old = await this.prisma.leaderboardSnapshot.findMany({
      where: { category }, orderBy: { createdAt: 'desc' }, skip: 24, select: { id: true },
    });
    if (old.length > 0) {
      await this.prisma.leaderboardSnapshot.deleteMany({ where: { id: { in: old.map((o) => o.id) } } });
    }
  }

  async getLeaderboard(category: string, page: number = 1, pageSize: number = 25) {
    const validCategory = CATEGORY_FIELD[category as LeaderboardCategory] ? category : 'volume';
    const snapshot = await this.prisma.leaderboardSnapshot.findFirst({
      where: { category: validCategory }, orderBy: { createdAt: 'desc' },
    });
    if (!snapshot) return { items: [], total: 0, page, pageSize, hasMore: false, category: validCategory };

    const allEntries = snapshot.entries as any[];
    const start = (page - 1) * pageSize;
    return {
      items: allEntries.slice(start, start + pageSize),
      total: allEntries.length, page, pageSize,
      hasMore: start + pageSize < allEntries.length,
      category: validCategory, updatedAt: snapshot.createdAt,
    };
  }
}
