...
export class ProgressionService {
  ...
  async handleBetXp(event: BetResolvedEvent) {
    // Wait for stats to be updated first
    await new Promise((r) => setTimeout(r, 50));

    let xpGain = 10; // base XP for placing a bet

    if (event.won) {
      xpGain += 25; // win bonus

      // Streak bonus: +10 per win if streak >= 3
    ...
    if (newLevel > updated.level) {
      ...
      this.logger.log(
        `Level up! User ${event.userId}: ${updated.level} → ${newLevel}`,
      );
  ...
  calculateLevel(xp: number): number {
    let level = 1;
    while (level < 100) {
      const required = this.xpForLevel(level + 1);
  ...
  getLevelsConfig() {
    ...
    for (let level = 1; level <= 100; level++) {
      config.push({
        ...
        xpToNext: this.xpForLevel(level + 1) - this.xpForLevel(level),
      });
    }
    return config;
  }
