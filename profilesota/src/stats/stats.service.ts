...
export class StatsService {
  ...
  async handleBetResolved(event: BetResolvedEvent) {
    ...
    // Upsert the bet record (idempotent)
    ...
    // Insert bet + update stats in a transaction
    await this.prisma.$transaction(async (tx) => {
      ...
      // Get current stats for streak/biggest-win calculation
      ...
      const newStreak = event.won
        ? currentStats.currentStreak + 1
        : 0;
      const newBestStreak = Math.max(currentStats.bestStreak, newStreak);
      const newBiggestWin = event.won && pnl.gt(currentStats.biggestWin)
        ? pnl
  ...
  async getBetHistory(
    ...
    if (status && status !== 'all') {
      where.outcome = status.toUpperCase();
    }
    ...
    return {
      items: items.map((bet) => ({
        ...
        rangeIndex: bet.rangeIndex,
        rangeLabel: bet.rangeLabel,
        amount: bet.amount.toString(),
        txHash: bet.txHash,
        outcome: bet.outcome,
        claimAmount: bet.claimAmount?.toString() || null,
        pnl: bet.pnl?.toString() || null,
        actualCount: bet.actualCount,
