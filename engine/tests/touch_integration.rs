...
//! signer + arena_index, and exercises the full flow:
//!
...
//! ARE deterministic: commit columns populated, the resolver records
//! the exact index path hash, ledger entries balance to the wei, and
...
use rush_engine::arena_index::{
    spawn_advancer as spawn_arena_advancer, ArenaIndex, RUSH_INDEX_SYMBOL,
};
...
use rush_engine::vrf::{SeedCipher, PATH_CONFIG_VERSION};
use sqlx::PgPool;
...
async fn build_engine(
    ...
    let vrf_cipher =
        Arc::new(SeedCipher::from_hex(CIPHER_KEY_HEX).expect("integration test cipher key"));
    let vault_addr = Address::from_str(VAULT_ADDR_HEX).expect("test vault");
    let commit_signer = Arc::new(
        WithdrawSigner::from_hex(SIGNER_HEX, 8453, vault_addr).expect("integration test signer"),
    );
...
fn hash_index_path_for_test(path: &[(i64, f64)]) -> String {
...
async fn placement_persists_commit_columns_and_locks_stake() {
    ...
    let window_start = now_ms + 100;
    let window_end = window_start + 3_000;
    ...
    assert!(
        path_hash.is_none(),
        "path_points_hash must be NULL pre-resolve"
    );
...
#[actix_rt::test]
async fn resolve_after_window_records_index_path_and_settles_status() {
    let pool = fresh_pool().await;
    let (engine, arena, _cipher, _signer, _signer_addr) = build_engine(pool.clone()).await;
    let stake = U256::from(10_000_000_000_000_000_u128); // 0.01 ETH
    let (user_id, _wallet) = make_user(&pool, "100000000000000000").await;
    ...
    // Resolution trace now populated.
    let resolved = engine.bet_repo().find_by_id(bet.id).await.unwrap().unwrap();
    assert!(
        resolved.revealed_seed.is_none(),
        "arena-index resolver has no per-bet seed"
    );
    let path_hash_db = resolved.path_points_hash.clone().expect("path_hash set");

    // The persisted hash must match the exact arena-index samples
    // available for the resolved window. This is the audit primitive
    // for the current single-line resolver.
    let replay = arena.path_window(resolved.window_start_ms, resolved.window_end_ms);
    assert!(
        !replay.is_empty(),
        "arena history exists for resolved window"
    );
    assert_eq!(
        hash_index_path_for_test(&replay),
        path_hash_db,
        "client-recomputed index path hash must equal engine's record"
    );
}
...
async fn resolve_before_window_end_rejected() {
    ...
    assert!(matches!(result, Err(TradingError::InvalidWindow { .. })));
}
...
async fn insufficient_balance_rejects_open() {
    ...
    let now_ms = chrono::Utc::now().timestamp_millis();
    let mut req = open_bet_for(user_id, &arena, stake, now_ms + 100, now_ms + 3_100);
    let q = engine
        .quote(
            &req.symbol,
            req.direction,
