...
export function ProfileEditor({ profile, onSave, isSaving }: ProfileEditorProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-lg mx-auto space-y-6">
      <div>
        <label className="block text-xs font-mono uppercase tracking-wider text-[#666666] mb-1">Avatar URL</label>
        <input type="url" value={avatarUrl} onChange={(e) => setAvatarUrl(e.target.value.slice(0, 500))} placeholder="https://..."
        ...
        <p className="text-[10px] font-mono text-[#666666] mt-1">Cole URL de imagem (gravatar, ENS, IPFS)</p>
      </div>
      <div>
      ...
      <button type="submit" disabled={isSaving || handleStatus === 'taken' || handleStatus === 'checking'}
